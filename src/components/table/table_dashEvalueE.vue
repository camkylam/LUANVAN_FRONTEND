<template>
    <div>
      <table class="my-table table-container">
        <thead>
          <tr>
          <th colspan="3" class="header-row"> đề nghị cấp ủy cơ sở nơi đảng viên công tác biểu dương hoặc xem xét xử lý theo quy định đối với đảng viên chưa thực hiện tốt.</th>
        </tr>
          <tr>
        ...
        <tbody>
          <tr>
            <td class="size-16">{{ countBieuduong}}</td>
            <td class="size-16">{{ countKhongdenghi }}</td>
            <td class="size-16">{{ countXemxetxuly }}</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  export default {
    ...
    methods: {
      updateCounts(evaluation) {
        this.countBieuduong = this.getCount('biểu dương', evaluation);
        this.countKhongdenghi = this.getCount('không đề nghị', evaluation);
        this.countXemxetxuly = this.getCount('xem xét xử lý', evaluation);
      },
