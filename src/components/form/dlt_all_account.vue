<template>
          ...
          <div class="d-flex justify-content-between mb-3">
            ...
            <div class="d-flex align-items-start">
              <button v-if="data.activeSelectAll == false" type="button" class="btn btn-light"
                ...
                <span id="add" class="mx-2" style="font-size: 15px">Chọn Tất Cả</span>
              </button>
              <button v-if="data.activeSelectAll == true" type="button" class="btn btn-light"
                @click="setDeleteAll(false), (data.activeSelectAll = false)">
                <span id="add" class="mx-2" style="font-size: 15px">Hủy Chọn</span>
              </button>
              <button type="button" class="btn btn-danger ml-3" data-toggle="modal" data-target="" @click="dltall">
                <span id="add" class="mx-2" style="font-size: 15px">Xóa Tất Cả</span>
              </button>
            </div>
          </div>
  ...
  'birthday',
  'registrationdate',
]" :activeAction="`false`" :borderTableAll="`true`"
          ...
          <Pagination :numberOfPages="data.numberOfPages" :totalRow="data.totalRow" :startRow="data.startRow"
            ...
            @update:currentPage="(value) => (data.currentPage = value)" class="mx-3" />
        </div>
      </div>
    </div>
  </div>
