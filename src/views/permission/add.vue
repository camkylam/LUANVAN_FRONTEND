        ...
        <div class="modal-body">
          <form action="" class="was-validated">
            <div class="form-group">
              <label for="">Loại quyền(<span style="color: red">*</span>):</label>
              <Select_Advanced :options="data.permissionTypesList.documents" style="height: 40px"
                :modelValue="data.modelValue" @chose="(value, value1) => {
                  if (value == 'other') {
                    createPT();
                  } else {
                    data.pTValue = value;
                  ...
                  " @delete="(value) => deletePT(value)" @searchSelect="(value) => refresh(value)" />
            </div>
            <div class="form-group" v-for="(value, index) in items">
              <div class="d-flex justify-content-between">
                <label for="name">Tên quyền {{ items.length == 1 ? "" : index + 1 }}(<span
                    style="color: red">*</span>):</label>
                <label for="" class="d-flex flex-row-reverse"><span @click="addInput"
                    class="material-symbols-outlined size-16 btn format-btn ml-2">
                    add </span><span class="material-symbols-outlined size-16 btn format-btn" @click="removeInput(index)">
                    remove
                  </span></label>
              </div>
              <input type="text" class="form-control" id="name" name="name" v-model="value.name" required />
            </div>
